...
@app.route("/")
def index():
    newsapi = NewsApiClient(api_key='50adc12789f742489caedf8d1546256c')
    ...
    if request.cookies.get("islogin"):
        return render_template("afterlogin.html" , context = mylist)
...
def afterlogin():
    ...
    if request.method == "GET":
        ...
        if email:
            if password:
                try:
                    db = sql.connect(host="localhost",port=3325,user="root",password="",database="project")
                    ...
                    if data:
                        ...
                        mylist = zip(news, desc, img)
                        return render_template("afterlogin.html",user=username,context=mylist)
                    else:
                        ...
                        #for i in password:
                            ...
                            if s>=1 and l>=1 and u>=1 and n>=1:
                                ...
                                try:
                                    ...
                                    return render_template("signup1.html",error = "Mail Send Successfully!!! Check you mail")
                            else:
...
def get_update():
    if request.method == "GET":
        ...
        headers = {
            ...
            'x-rapidapi-key': "68c4ab90a4msh79877078d73a178p190a7ejsn7e49f98197fb"
         }

        response = requests.request("GET", url, headers=headers)

        data=response.json()
        return render_template("coronaupdate.html",data=data)
    else:
         return render_template("afterlogin.html",error="Invalid Method")                
